...
def TCR(wb, k, p):
    ...
    r = math.sqrt((wb["X"] - wb["Xp"]) ** 2 + (wb["Y"] - wb["Yp"]) ** 2 + (wb["H"] - wb["Hnorm"]) ** 2) 
    #spherical geocentric radius
    # integration over three variables
    eq = (wb["Xp"] * math.log(wb["Yp"] + r) + wb["Yp"] * math.log(wb["Xp"] + r) 
            - ((wb["Hnorm"] * math.atan(wb["Xp"] * wb["Yp"] / (wb["Hnorm"] * r))) 
            - (wb["X"] * math.log(wb["Y"] + r) + wb["Y"] * math.log(wb["X"] + r)
            - wb["H"] * math.atan(wb["X"] * wb["Y"] / (wb["H"] * r)))))
...
# mathematical formula #3.1
for n in range(len(wb)):
    eq_values.append(TCR(wb.iloc[n], k, p))
